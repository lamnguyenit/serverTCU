...
function theme_klass_process_css($css, $theme) {
    ...
    $css = theme_klass_set_customcss($css, $customcss);
		...
		$css = theme_klass_set_fontwww($css);
 ...
 * @param $css
	* @return string
 */
function theme_klass_set_fontwww($css) {
    global $CFG, $PAGE;
...
{	
				function get_logo_url($type='header')
				{
								...
								}else if($type == "bgfooter")
                                {
                                                 $logo = $theme->setting_file_url('bgfooter', 'bgfooter');
                                                $logo = empty($logo)?$OUTPUT->pix_url('home/bgfooter', 'theme'):$logo;
