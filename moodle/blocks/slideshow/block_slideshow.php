...
class block_slideshow extends block_base {
    ...
    public function get_required_javascript() {
        ...
        $this->page->requires->js('/blocks/slideshow/js/cycle2.js');
        $this->page->requires->js('/blocks/slideshow/js/flip.js');
        $this->page->requires->js('/blocks/slideshow/js/center.js');
        $this->page->requires->js('/blocks/slideshow/js/scrollVert.js');
        $this->page->requires->js('/blocks/slideshow/js/shuffle.js');
        $this->page->requires->js('/blocks/slideshow/js/tile.js');
        $this->page->requires->js('/blocks/slideshow/js/carousel.js');
	...
	public function get_content() { 
		...
		if (!empty($this->config->height)) {
	    	$this->content->height = $this->config->height . 'px';
			$imageheight = ' data-cycle-auto-height="false"';
		} else {
			$this->content->height = 'auto';
			$imageheight = ' data-cycle-auto-height="calc"';
		}
		...
		if (!empty($this->config->slidespeed)) {
			...
			$this->content->text = '<div id="page-slideshow">
							...
							data-cycle-fx = "' . $this->content->transition . '"
							data-cycle-speed = "' . $this->content->slidespeed . '"
							data-cycle-timeout = "' . $this->content->slidedelay . '"
	...
	. '>';
			...
			foreach ($images as $image) {
				...
				$this->content->text .= '<img src="' . $url . '"'. $imageheight .' />' ;
			}
			
			$this->content->text .= '</div></div><div style="clear:both;"><div class="center">
</div></div>';
			...
			$script = "
				...
				$(window).load(function(){
					$('.cycle-slideshow').each(function(){
						...
						if (maxWidth < $('#page-slideshow').innerWidth() ) {
						  $(this).css({'width':maxWidth});
						} else {
						  $(this).find('img').css({'width':$('#page-slideshow').innerWidth()*.95});
						}
					});       
				});
...
$('.block.block_slideshow').css({'margin':'0','border':'0','clear':'both','border-radius':'0','padding':'0','background':'" . $ssbackground . "'});";						
			

			} else {
				$script .= "
				$('.cycle-slideshow').css('max-width', $('#page-slideshow').innerWidth() );
				$('.cycle-slideshow img').css('max-width', $('#page-slideshow').innerWidth() * .9);
				";
			}

				if($this->content->height != 'auto'){
					$script .= "
					$('.cycle-slideshow').css('height', '" . $this->content->height . "');
					$('.cycle-slideshow img').css('height', '" . $this->content->height . "');
					";
				}
